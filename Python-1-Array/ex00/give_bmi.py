def give_bmi(height: list[int | float],
    ...
    if (all(isinstance(h, int) for h in height) and
        ...
        is_int = True
    elif (all(isinstance(h, float) for h in height) and
            all(isinstance(w, float) for w in weight)):
        is_int = False
    else:
        raise ValueError("Mismatched types in lists")
    if len(height) != len(weight):
        print("""Warning: Lists do not have the same length.
            The last values of the longer list are ignored.""")
    bmi = []
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    if all(isinstance(value, (int, float)) for value in bmi):
        ...
        raise ValueError("""Passed list must be consistantly
